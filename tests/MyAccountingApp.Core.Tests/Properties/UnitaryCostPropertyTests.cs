...
{
    [Property]
    public Property UnitaryCost_MultipliedByQuantity_EqualsTransactionAmount(
        decimal amount, decimal quantity, string currencyCode)
    {
        ...
        return (Math.Abs(totalFromCost - amount) < 0.01m).ToProperty();
    }

    [Property]
    public Property UnitaryCost_IsAlwaysPositive(decimal amount, decimal quantity)
    {
        amount = Math.Abs(amount);
        if (amount == 0) amount = 100;
        quantity = Math.Abs(quantity);
        ...
        return (assetTx.UnitaryCost().Amount > 0).ToProperty();
    }

    [Property]
    public Property UnitaryCost_WithQuantityOne_EqualsAmount(decimal amount)
    {
        amount = Math.Abs(amount);
        if (amount == 0) amount = 100;
        ...
        return (assetTx.UnitaryCost().Amount == roundedAmount).ToProperty();
    }
}
